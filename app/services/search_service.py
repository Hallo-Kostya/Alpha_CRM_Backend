...
class SearchService:
    ...
    async def search_entities(
        ...
        project_query = (
            ...
            .where(ProjectModel.name.ilike(f"%{query}%"))
            .limit(limit)
        )
        ...
        team_query = (
            ...
            .where(TeamModel.name.ilike(f"%{query}%"))
            .limit(limit)
        )
        ...
        student_query = (
            select(
                ...
                StudentModel.id,
                func.concat(StudentModel.first_name, " ", StudentModel.last_name).label(
                    "name"
                ),
...
def search_service_getter(
    ...
    return SearchService(project_repo, team_repo, student_repo, curator_team_repo)
