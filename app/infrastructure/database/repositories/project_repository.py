...
class ProjectRepository(BaseRepository[ProjectModel]):
    ...
    async def get_projects_with_excluded_ids(self, excluded_ids: list[UUID], filters: dict[str, Any]) -> list[ProjectModel]:
        query = select(ProjectModel).where(ProjectModel.id.notin_(excluded_ids)).filter_by(**filters)
        result = await self.session.execute(query)
